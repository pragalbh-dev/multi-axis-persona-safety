...
- [ ] T4.1: Implement per-PC steering experiments
  ...
  - For each (subject, PC) under evaluation:
    ...
    - **Sample budget (compute cut):** run the **full 1,100 prompts only at λ=0 (baseline) and at the strongest-effect strength** (determined after a quick λ∈{-2, +2} pilot on 200 prompts). For the remaining 7 strengths, use a **stratified 500-prompt subsample** of the 1,100 (stratified across the **13 OpenAI-policy harm categories** shared by DAN + Shah-reconstructed, ~38 prompts/category). Saves ~55% of generations while preserving curve shape.
    - **Dual-dataset rule (per CONVENTIONS "Jailbreak datasets"):** every condition runs against BOTH the DAN primary set and the Shah-reconstructed secondary set, results tagged by `dataset` column. λ=0 + strongest-strength = 1,100 each = 2,200 prompts/subject/PC. Intermediate strengths = 500 stratified each = 1,000 prompts/subject/PC. Cross-dataset agreement on the blind-spot finding is a key replication check; we report DAN harm-rate, Shah-reconstructed harm-rate, and the delta in the H1 result tables.
